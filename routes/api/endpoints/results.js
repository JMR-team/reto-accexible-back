...
router.post('/', authenticateUser, validateResults, async function (req, res) {
    ...
    let conversationString = req.body.chatBotAnswers
        .map(({ text }) => text)
        .join(' ')
        .toLowerCase();
    chatScore += responseTimeScore(
        req.body.chatBotAnswers.map(({ text, responseTime }) => ({
            text,
            responseTime,
        }))
    ); //response time
    chatScore += [rumination, personalPronouns, keyWords]
        .map((scoringFunction) => scoringFunction(conversationString))
...
function authenticateUser(req, res, next) {
    ...
    if (token != undefined) {
        ...
        // Logic to implement if the request does not contain a token
        // Check validity of email format, reject requests with invalid email address
        if (!emailValidator.validate(req.body.email)) {
            return res
                .status(400)
                .json({ err: 'invalid email address format' });
        }
            ...
            .then((user) => {
                if (user != undefined)
                    return res.status(403).json({
                        err: 'Forbidden, email belongs to a registered user',
                    });
                next();
            });
    }
...
function validateResults(req, res, next) {
    ...
    if (
        isNaN(req.body.testScore) ||
        req.body.testScore < 2.25 ||
        req.body.testScore > 9
    ) {
        return res
            .status(400)
            .json({ err: 'testScore must be a float between 2.25 and 9' });
    }
